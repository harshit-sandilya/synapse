...
from .bench_models import build_three_layer_lif_model_ir
...
class BenchmarkApiClient:
    ...
    def experiment_telemetry(self, experiment_id: str) -> dict[str, Any]:
        return self._get(f"/api/v1/experiment/{experiment_id}/telemetry")
    ...
    def wait_for_telemetry_ready(
        ...
        *,
        ...
        deadline = time.time() + timeout_seconds
        while time.time() < deadline:
            payload = self.experiment_telemetry(experiment_id)
        ...
        raise TimeoutError(
            f"Timed out waiting for telemetry publisher for experiment {experiment_id}"
        )
