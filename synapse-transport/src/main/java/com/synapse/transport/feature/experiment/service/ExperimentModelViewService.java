...
public class ExperimentModelViewService {
    ...
    public ExperimentModelResponse getModel(UUID experimentId) {
        ...
        if (modelConfig == null) {
            return ExperimentModelResponse.builder()
                ...
                .inputShape(datasetConfig == null ? null : datasetConfig.getInputShape())
                .outputShape(datasetConfig == null ? null : datasetConfig.getOutputShape())
                .build();
        }

        JsonNode modelIr =
            modelConfig.getModelArtifact() == null
                ? null
                : experimentArtifactService.readArtifact(modelConfig.getModelArtifact());

        JsonNode trainingConfig =
            modelConfig.getModelConfigArtifact() == null
                ? null
                : experimentArtifactService.readArtifact(modelConfig.getModelConfigArtifact());
        ...
        return ExperimentModelResponse.builder()
            ...
            .optimizer(
                trainingConfig != null && trainingConfig.hasNonNull("optimizer")
                    ? com.synapse.transport.common.enums.OptimizerType.valueOf(trainingConfig.get("optimizer").asString())
                    : null
            )

            .lossFunction(
                trainingConfig != null && trainingConfig.hasNonNull("lossFunction")
                    ? com.synapse.transport.common.enums.LossFunctionType.valueOf(trainingConfig.get("lossFunction").asString())
                    : null
            )

            .learningRate(
                trainingConfig != null && trainingConfig.hasNonNull("learningRate")
                    ? trainingConfig.get("learningRate").asDouble()
                    : null
            )

            .epochs(trainingConfig != null && trainingConfig.hasNonNull("epochs") ? trainingConfig.get("epochs").asInt() : null)

            .lastTrainingError(modelConfig.getLastTrainingError())
