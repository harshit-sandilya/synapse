...
public class MetricsExtractionService {
    ...
    public void extractAndPersistMetrics(Experiment experiment, String metricsStorageKey) {
        if (metricsStorageKey == null || metricsStorageKey.isBlank()) {
            return;
        }
        ...
        try {
            root = storageService.readLastJsonLine(metricsStorageKey);
        ...
        if (experiment.getTaskType() == ExperimentTaskType.CLASSIFICATION) {
            ClassificationMetrics entity = classificationMetricsRepository
                ...
                .orElse(ClassificationMetrics.builder().experiment(experiment).build());

            entity.setAccuracy(metrics.path("accuracy").asDouble());

            entity.setPrecisionScore(metrics.path("precision").asDouble());

            entity.setRecallScore(metrics.path("recall").asDouble());

            entity.setF1Score(metrics.path("f1").asDouble());

            classificationMetricsRepository.save(entity);
        ...
        RegressionMetrics entity = regressionMetricsRepository
            .findByExperimentId(experiment.getId())
            .orElse(RegressionMetrics.builder().experiment(experiment).build());

        entity.setMse(metrics.path("mse").asDouble());

        entity.setRmse(metrics.path("rmse").asDouble());

        entity.setMae(metrics.path("mae").asDouble());

        entity.setR2Score(metrics.path("r2").asDouble());

        regressionMetricsRepository.save(entity);
    }
}
